...
catboost = CatBoostClassifier(eval_metric='Accuracy',iterations=500,depth=10,learning_rate=0.1,l2_leaf_reg=1,random_state=42, verbose=0)
...
if uploaded_file is not None:
    ...
    # Predict Attrition
    user_df['Attrition'] = catboost.predict(user_df)
    ...
    if st.button('Predict'):
        user_prediction = catboost.predict(user_df)
        user_prediction_prob = catboost.predict_proba(user_df)[:, 1]
